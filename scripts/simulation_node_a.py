...
from assignment_2_2023.msg import Custom, Distance
...
def main():
    ...
    pub1=rospy.Publisher("/dist_min_obs", Distance, queue_size=10)
    # create the subscriber to the topic /odom, of type Odometry, and the created callback function
    rospy.Subscriber("/odom", Odometry, callback)
    rospy.Subscriber("/scan", LaserScan, msg_cb)
    rospy.wait_for_service('/last_target')
    service_proxy=rospy.ServiceProxy('/last_target', Last_target)
    response=service_proxy()
    print("Response of the service: ", response)
