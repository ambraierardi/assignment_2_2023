...
from assignment_2_2023.msg import Custom, Goal
import sys
import select
import actionlib
...
def action_client():
    ...
    while not rospy.is_shutdown():   
    	...
    	cheeck=int(input("1 for yes: "))
    	if check==1:
    		print("insert a new size window pls: ")
    		wdwsz=int(input(">>: "))
    		rospy.set_param("window_size", wdwsz)
        ...
        while act_cl.get_state() != actionlib.GoalStatus.SUCCEEDED:
            ...
            if user_input=='c':
				...
				prx=rospy.ServiceProxy('/gazebo/reset_world',Empty)
				response=prx()
				break
...
def cb(msg):
	...
	new_msg.x=msg.pose.pose.position.x
	new_msg.y=msg.pose.pose.position.y
	new_msg.z=msg.pose.pose.position.z
...
def main():
    ...
    pub1 = rospy.Publisher("/current_goal", Goal, queue_size=10)
    # create the subscriber to the topic /odom, of type Odometry, and the created callback function
    rospy.Subscriber("/odom", Odometry, callback)
    rospy.Subscriber("/reaching_goal/goal", PlanningActionGoal, cb)
